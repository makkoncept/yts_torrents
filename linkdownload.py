...
def download_torrent(bin_content, movie_name, type):
    ...
    if '/' in torrent_name:
        ...
        #print("{} already downloaded".format(torrent_name)) EDIT: Now done before getting info from server for speed.
        #return
    ...
    for movie in movies:
        ...
        if bluray_1080 is not None:
            if (movie + 'bluray_1080p.torrent') in cache:
                ...
                response = requests_retry_session().get(bluray_1080, timeout=120)
                download_torrent(response.content, movie, 'bluray_1080p')
                continue
        elif web_1080 is not None:
            if (movie + 'web_1080p.torrent') in cache:
                print("{} already downloaded".format(movie))
                #EDIT: Already exists, don't waste time requesting info from server.
            else:
                response = requests_retry_session().get(web_1080, timeout=120)
                download_torrent(response.content, movie, 'web_1080p')
                continue
        elif bluray_720 is not None:
            if (movie + 'bluray_720p.torrent') in cache:
                print("{} already downloaded".format(movie))
                #EDIT: Already exists, don't waste time requesting info from server.
            else:
                response = requests_retry_session().get(bluray_720, timeout=120)
                download_torrent(response.content, movie, 'bluray_720p')
                continue
        elif web_720 is not None:
            if (movie + 'web_720p.torrent') in cache:
                print("{} already downloaded".format(movie))
                #EDIT: Already exists, don't waste time requesting info from server.
            else:
